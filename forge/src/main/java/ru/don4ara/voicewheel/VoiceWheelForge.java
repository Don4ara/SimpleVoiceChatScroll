...
public final class VoiceWheelForge {
    ...
    public VoiceWheelForge() {
        MinecraftForge.EVENT_BUS.addListener(VoiceWheelForge::onMouseScroll);
        MinecraftForge.EVENT_BUS.addListener(VoiceWheelForge::onRenderPlayer);
    }
    ...
    private static boolean handleScroll(double scroll) {
        ...
        double previous = VoicechatClient.VOLUME_CONFIG.getPlayerVolume(state.getUuid());
        double step = previous >= 1.0D ? 0.1D : 0.05D;
        double volume = Mth.clamp(previous + step * scroll, 0.0D, 4.0D);
        VoicechatClient.VOLUME_CONFIG.setPlayerVolume(state.getUuid(), volume);
